...
function classify(item) {
  ...
  for (const member of members.members) {
    ...
    const group = keywords.groups.find((item) => item.name === member.group);
    const hasGroupKeyword = Boolean(group?.keywords?.some((word) => text.includes(word)));
    if (names.some((name) => shouldMatchMemberName(name, text, hasGroupKeyword))) {
...
function shouldMatchMemberName(name, text, hasGroupKeyword) {
  const normalized = String(name || "").normalize("NFKC");
  if (!normalized || !text.includes(normalized)) return false;
  const isShortKatakana = /^[ァ-ヴー]{2,4}$/.test(normalized);
